...
extension CoreInputDelta {
    @inlinable
    func setInput(
        _ keys: inout UInt32, input: CoreInput, flag: UInt32, isPressed: Bool, useValue: Bool = true
    ) {
        let isPressed: UInt32 = isPressed ? 1 : 0
        keys =
            self.input.contains(input) && useValue
...
public struct mGBAEclipseCoreSettings: CoreSettings {
    ...
    public static let descriptor: CoreSettingsDescriptor<mGBAEclipseCoreSettings> = .init(
        sections: [])
...
public final class mGBAEclipseCore: CoreProtocol {
    ...
    public static let id: String = "dev.magnetar.eclipseemu.mgba"
    ...
    public init(
        ...
        self.samples = .allocate(
            capacity: MemoryLayout<Int16>.stride * sampleCount * Int(audioSampleRate))
    }
    ...
    public func writeInput(_ delta: CoreInputDelta, for player: UInt8) {
        ...
        delta.setInput(
            &keys, input: .faceButtonRight, flag: 1 << GBA_KEY_A.rawValue,
            isPressed: delta.isPressed)
        delta.setInput(
            &keys, input: .faceButtonDown, flag: 1 << GBA_KEY_B.rawValue, isPressed: delta.isPressed
        )
        delta.setInput(
            &keys, input: .leftShoulder, flag: 1 << GBA_KEY_L.rawValue, isPressed: delta.isPressed)
        delta.setInput(
            &keys, input: .rightShoulder, flag: 1 << GBA_KEY_R.rawValue, isPressed: delta.isPressed)
        delta.setInput(
            &keys, input: .start, flag: 1 << GBA_KEY_START.rawValue, isPressed: delta.isPressed)
        delta.setInput(
            &keys, input: .select, flag: 1 << GBA_KEY_SELECT.rawValue, isPressed: delta.isPressed)
        delta.setInput(
            &keys, input: .dpad, flag: 1 << GBA_KEY_UP.rawValue, isPressed: delta.isUp,
            useValue: delta.useY)
        delta.setInput(
            &keys, input: .dpad, flag: 1 << GBA_KEY_DOWN.rawValue, isPressed: delta.isDown,
            useValue: delta.useY)
        delta.setInput(
            &keys, input: .dpad, flag: 1 << GBA_KEY_LEFT.rawValue, isPressed: delta.isLeft,
            useValue: delta.useX)
        delta.setInput(
            &keys, input: .dpad, flag: 1 << GBA_KEY_RIGHT.rawValue, isPressed: delta.isRight,
            useValue: delta.useX)
        self.inputState = keys
        core.pointee.setKeys(core, keys)
    }

    public func setCheats(cheats: [CoreCheat]) {
        for cheat in cheats {
            if let set = cheatSets[cheat] {
                set.pointee.enabled = true
                return
            }
            ...
            if size > 0 {
                set.pointee.copyProperties(
                    set, mCheatSetsGetPointer(&device.pointee.cheats, size - 1).pointee)
            }

            for line in cheat.code.trimmingCharacters(in: .whitespacesAndNewlines).split(
                separator: "\n")
            {
